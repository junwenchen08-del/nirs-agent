...
class PreprocessingPipeline:
    ...
    # ------------------------------------------------------------------
    def fit(
        self, X: np.ndarray, wv: np.ndarray | None = None
    ...
    def transform(
        ...
        if has_stateful and not self._fit_state:
            ...
            warnings.warn(
                "PreprocessingPipeline.transform() called without fit() on a "
                "pipeline containing stateful steps; falling back to apply() "
                "which uses X's own statistics. Call fit() on training data "
    ...
    def _check_method(method: str) -> None:
        if method not in PRESTEP_METHODS:
            raise KeyError(
                ...
                f"Available: {sorted(PRESTEP_METHODS.keys())}"
            )
