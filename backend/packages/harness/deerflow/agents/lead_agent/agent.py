    ...
    *,
    ...
    nir_route = SkillAutoRoute(
        ...
        patterns=(
            ...
            r"(?:多成分|多组分).*(?:分析|预测|建模|光谱)",
            r"(?:同时分析|同时预测).*(?:成分|组分)",
            r"(?:近红外|NIR|光谱).*(?:定性|分类|判别|鉴别|真伪|品种|产地)",
            r"(?:定性|分类|判别|鉴别|真伪|品种|产地).*(?:近红外|NIR|光谱)",
            r"\b(?:PLS-?DA|SIMCA)\b",
            r"\b(?:multiple components|simultaneous analysis|simultaneous prediction)\b",
        ),
