...
_DEFAULT_KNOWN_DOMAINS = frozenset({"food_moisture", "food_protein", "pharma", "feed", "soil", "default"})
_KNOWN_DOMAINS = _DEFAULT_KNOWN_DOMAINS


def _known_domains() -> frozenset[str]:
    configured = os.environ.get("NIR_KNOWN_DOMAINS", "")
    if not configured:
        return _DEFAULT_KNOWN_DOMAINS
    domains = {part.strip() for part in configured.split(",") if part.strip()}
    return frozenset(domains) if domains else _DEFAULT_KNOWN_DOMAINS
    ...
    *,
    ...
    if domain and domain not in known_domains:
        return {
            ...
            "reason": (f"domain='{domain}' 不在已知领域列表内 ({sorted(known_domains)})，建议先检索论文库了解该领域的常用预处理方法和合理指标范围。"),
        }
    ...
    if grade in {"C", "D", "F"} and attempt is not None and attempt >= 2:
        if trend == "upward":
            return {
                ...
                "reason": (f"重试 {attempt} 次仍未通过门禁 (grade={grade})，残差呈上升趋势，建议检索论文库确认该领域的基线校正最佳实践。"),
            }
        if trend == "downward":
            return {
                "should_search": True,
                "query": f"snv vs msc scatter correction {domain} NIR",
                "reason": (f"重试 {attempt} 次仍未通过门禁 (grade={grade})，残差呈下降趋势，建议检索论文库确认该领域的散射校正方法选择。"),
            }
        if variance == "high":
            return {
                "should_search": True,
                "query": f"sg_smooth window selection {domain} NIR noise",
                "reason": (f"重试 {attempt} 次仍未通过门禁 (grade={grade})，残差方差高，建议检索论文库确认该领域的平滑窗口选择经验。"),
            }
        return {
            "should_search": True,
            "query": f"{domain} NIR PLS improve RPD preprocessing",
            "reason": (f"重试 {attempt} 次仍未通过门禁 (grade={grade})，建议检索论文库寻找该领域提升 RPD 的预处理组合经验。"),
        }
    ...
    if r2_val is not None and r2_val < 0.7:
        return {
            ...
            "reason": (f"R²_val={r2_val:.3f} 偏低，建议检索论文库确认 {domain} 领域的合理 R²/RPD 范围，判断是否数据本身问题还是预处理不当。"),
        }
