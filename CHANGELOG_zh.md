...
- **NIR 预处理：** 新增稳健 SNV、可在训练边界拟合并回放的 EMSC、孤立尖峰去除、
  Norris-Williams 一阶/二阶导数，均以显式流水线方法提供。
- **NIR 波长轴：** 新增独立波长对齐工具，同时更新光谱和波长轴，校验严格单调性，
  记录轴哈希，并默认禁止外推。
- **NIR 预处理目录：** 新增统一运行时目录、只读的清单/详情/候选推荐工具，以及始终
  包含原始光谱基线的校准集内有界候选生成。
- **Chemotools 实现：** 固定使用 Chemotools 0.4.4，并为 SNV、MSC、SG 平滑、
  EMSC、RNV、Whittaker 平滑、中值滤波、SG/Norris-Williams 导数、airPLS/ArPLS/asLS、
  去趋势、橡皮筋基线校正和支持的缩放方法增加经过数值验证的适配器。新工件记录准确实现
  版本，旧工件继续固定使用原生实现。
- **NIR 校准迁移：** 新增独立的配对样本 DS/PDS/SST 工具族，固定目标到参考方向，绑定
  仪器和波长轴，记录内部留出证据，可选验证参考模型 RMSEP，并使用防篡改工件清单；
  只有不重叠的独立配对验证集和参考模型 RMSEP 门禁同时通过，才批准生产使用。

### 变更

- **NIR 工具契约：** `nir_preprocess` 复用核心流水线校验并返回实现证据，同时同步工具、
  工作流策略、技能和包文档。单次自动回归分析改用有界候选和仅基于校准集的
  RMSECV/1% 简单性规则。
